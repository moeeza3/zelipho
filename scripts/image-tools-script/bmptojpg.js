...
// Convert a BMP image to JPG using canvas
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        (blob) => {
          ...
          const name = file.name.replace(/\.bmp$/i, ".jpg");
    ...
    const next = () => {
      ...
      while (inProgress < maxConcurrent && queue.length > 0) {
        ...
        if (!file.name.toLowerCase().endsWith(".bmp")) {
          errorMessage.textContent = "Only BMP files are allowed.";
...
uploadInput.addEventListener("change", async (e) => {
  ...
  // Large file warning
  const hasLarge = files.some(f => f.size > 2 * 1024 * 1024);
  if (hasLarge) {
